    ...
    createFirstTimer: async function (payload) {
      ...
      if (client) {
        try {
          ...
          var isUuid = function (v) { return /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(String(v || "")); };
          var row = {
            id: payload.id,
            first_timer_number: payload.first_timer_number || payload.id,
            full_name: full,
            first_name: payload.nome || payload.first_name || full.split(" ")[0] || "",
            last_name: payload.apelido || payload.last_name || full.split(" ").slice(1).join(" ") || "",
            title: payload.tratamento || payload.title || null,
            gender: payload.genero || payload.gender || null,
            date_of_birth: payload.data_de_nascimento || payload.date_of_birth || null,
            phone: payload.telefone || payload.phone || null,
            whatsapp: payload.whatsapp || payload.telefone || payload.phone || null,
            email: payload.email || null,
            address: payload.endereco || payload.address || null,
            neighborhood: payload.neighborhood || payload.bairro || payload.endereco || null,
            profession: payload.profession || payload.profissao || null,
            church_id: payload.church_id && isUuid(payload.church_id) ? payload.church_id : null,
            church_name: payload.church_name || payload.igreja || null,
            cell_group_id: payload.cell_group_id && isUuid(payload.cell_group_id) ? payload.cell_group_id : null,
            cell_group_name: payload.cell_group_name || payload.grupo_de_celula || null,
            cell_id: payload.cell_id && isUuid(payload.cell_id) ? payload.cell_id : null,
            ...
            invited_by: payload.invited_by_name || payload.invited_by || payload.convidado_por || null,
            invited_by_name: payload.invited_by_name || payload.invited_by || payload.convidado_por || null,
