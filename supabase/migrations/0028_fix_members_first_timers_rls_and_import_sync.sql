...
-- 1. Grant permissions to authenticated, anon and service_role if they exist
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'authenticated') THEN
    ...
    GRANT ALL ON TABLE public.foundation_students TO authenticated;
  END IF;
  IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'anon') THEN
    ...
    GRANT ALL ON TABLE public.foundation_students TO anon;
  END IF;
  IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'service_role') THEN
    ...
    GRANT ALL ON TABLE public.foundation_students TO service_role;
  END IF;
END $$;
...
-- Create policies dynamically depending on whether roles exist
DO $$
BEGIN
...
-- 6. Record migration in schema_meta
