...
export function normalizeCell(input: Partial<Cell> & { id?: string }): Cell {
  ...
  return {
    ...
    groupId,
    cell_group_name: input.cell_group_name || input.group_name || CELL_GROUPS_SEED.find((g) => g.id === groupId)?.name || "",
    group_name: input.group_name || input.cell_group_name || CELL_GROUPS_SEED.find((g) => g.id === groupId)?.name || "",
...
export function normalizeCellReport(
  ...
  return {
    ...
    submitted_by_cell_role: input.submitted_by_cell_role || "",
    authorized_cell_id: input.authorized_cell_id || input.cell_id || null,
    auth_required: input.auth_required !== false,
    submission_source: input.submission_source || "cell_leader_portal",
    status,
