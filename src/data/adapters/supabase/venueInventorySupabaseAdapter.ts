...
function mapInventoryItemToRow(item: Partial<InventoryItem>, forUpdate = false): SupabaseRow {
  const row: SupabaseRow = {
    ...
    name: item.nome_do_item || item.name || null,
    description: item.observacoes || item.description || null,
    ...
    unit: item.unit || "unit",
    church_id: uuidOrNull(item.igreja || item.church_id),
    church_name: item.church_name || null,
    department_id: item.department_id != null ? String(item.department_id) : null,
    department_name: item.departamento_responsavel || item.department_name || null,
    space_id: uuidOrNull(item.space_id),
    space_name: item.localizacao || item.space_name || null,
    ...
    acquisition_date: item.data_de_entrada || item.acquisition_date || todayIso(),
    acquisition_cost: Number(item.valor_unitario ?? item.acquisition_cost ?? 0),
    ...
    status: item.estado || item.status || "Available",
    condition: item.condition || "Good",
    location_notes: item.location_notes || null,
    usage_notes: item.observacoes || item.usage_notes || null,
...
function mapMovementToRow(movement: Partial<InventoryMovement>, forUpdate = false): SupabaseRow {
  const row: SupabaseRow = {
    ...
    item_name: movement.item || movement.item_name || null,
    ...
    from_space_name: movement.origem || movement.from_space_name || null,
    ...
    to_space_name: movement.destino || movement.to_space_name || null,
    to_user_id: uuidOrNull(movement.to_user_id),
    to_user_name: movement.to_user_name || null,
    quantity: Number(movement.quantidade ?? movement.quantity ?? 1),
    reason: movement.reason || null,
    notes: movement.observacoes || movement.notes || null,
    movement_date: movement.data_de_saida || movement.movement_date || nowIso(),
    performed_by: uuidOrNull(movement.performed_by_user_id),
    performed_by_name: movement.pessoa_responsavel || movement.performed_by_name || null,
    approved_by: uuidOrNull(movement.approved_by_user_id),
    approved_by_name: movement.aprovado_por || movement.approved_by_name || null,
    approved_at: movement.approved_at || null,
    status: movement.estado || movement.status || "Completed",
    metadata: {},
  };
...
function mapMaintenanceToRow(record: Partial<InventoryMaintenanceRecord>, forUpdate = false): SupabaseRow {
  const row: SupabaseRow = {
    ...
    item_name: record.item || record.item_name || null,
    issue_title: record.problema_reportado || record.issue_title || null,
    issue_description: record.problema_reportado || record.issue_description || null,
    ...
    assigned_to_name: record.tecnico_ou_responsavel || record.assigned_to_name || null,
    repair_vendor: record.repair_vendor || null,
    estimated_cost: Number(record.custo_da_reparacao ?? record.estimated_cost ?? 0),
    actual_cost: Number(record.custo_da_reparacao ?? record.actual_cost ?? 0),
    currency: record.currency || "MZN",
    status: record.estado || record.status || "Reported",
    priority: record.priority || "Normal",
    started_at: record.data_de_envio || record.started_at || null,
    completed_at: record.data_de_retorno || record.completed_at || null,
    resolution_notes: record.observacoes || record.resolution_notes || null,
...
function mapVenueSpaceToRow(space: Partial<VenueSpace>, forUpdate = false): SupabaseRow {
  const row: SupabaseRow = {
    church_id: uuidOrNull(space.igreja || space.church_id),
    church_name: space.church_name || null,
    name: space.nome_do_espaco || space.name || null,
    description: space.observacoes || space.description || null,
    space_type: space.tipo || space.space_type || null,
    capacity: Number(space.capacidade ?? space.capacity ?? 0),
    responsible_user_id: uuidOrNull(space.responsible_user_id),
    responsible_name: space.responsavel || space.responsible_name || null,
    status: space.estado || space.status || "Available",
    notes: space.observacoes || space.notes || null,
    metadata: {},
  };
...
function mapChecklistToRow(checklist: Partial<ServiceChecklist>, forUpdate = false): SupabaseRow {
  const row: SupabaseRow = {
    church_id: uuidOrNull(checklist.igreja || checklist.church_id),
    church_name: checklist.church_name || null,
    service_name: checklist.tipo_de_culto_ou_evento || checklist.service_name || null,
    service_date: checklist.data_do_culto || checklist.service_date || todayIso(),
    ...
    responsible_name: checklist.responsavel || checklist.responsible_name || null,
    ...
    issues_found: checklist.observacoes || checklist.issues_found || null,
    actions_taken: checklist.actions_taken || null,
    status: checklist.estado || checklist.status || "Open",
