...
// Template path provided by user
...
console.log(`Template columns count: ${templateColumns.length}`);

console.log(`Reading Excel file: ${EXCEL_PATH}...`);
const workbook = XLSX.readFile(EXCEL_PATH, { raw: false });
console.log(`Workbook loaded. Sheets found: ${workbook.SheetNames.length}`);
...
console.log(`Total existing members in Supabase DB: ${supabaseMembers.length}`);
...
console.log(`\nExtracted ${excelRows.length} missing members ready for template CSV output.`);
...
const csvLines = [templateHeaderLine];
...
excelRows.forEach((m, idx) => {
  ...
  const lineValues = templateColumns.map(col => escapeCsv(rowMap[col] ?? ""));
  csvLines.push(lineValues.join(","));
});

const outputPath = "C:/Users/Alves King Edition/Documents/Project 2/ce-mozambique-dashboard/missing_members_supabase_template_import.csv";
fs.writeFileSync(outputPath, csvLines.join("\n"), "utf-8");

console.log(`\n✅ Generated clean template-compliant CSV file at:\n${outputPath}`);
console.log(`Total missing member rows: ${csvLines.length - 1}`);

try {
  const defaultPath = "C:/Users/Alves King Edition/Documents/Project 2/ce-mozambique-dashboard/missing_members_supabase_import.csv";
  fs.writeFileSync(defaultPath, csvLines.join("\n"), "utf-8");
  console.log(`✅ Also updated default file at:\n${defaultPath}`);
} catch (e) {
  console.log(`Note: missing_members_supabase_import.csv is currently open in editor. File saved as missing_members_supabase_template_import.csv`);
}
