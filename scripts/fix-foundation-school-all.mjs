...
// 1. Update hydrateFoundationSchoolFromRepository
const targetHydrate = `    // 2. Teachers
...
const replaceHydrate = `    // 2. Teachers
    let teachersData = null;
    if (sbClient) {
      const res = await sbClient.from("foundation_school_teachers").select("*").order("created_at", { ascending: false });
      if (res?.data && Array.isArray(res.data)) teachersData = res.data;
...
if (dashCode.includes(targetHydrate)) {
  dashCode = dashCode.replace(targetHydrate, replaceHydrate);
  console.log("Hydrate section updated successfully.");
} else {
  console.warn("Hydrate target not found directly, checking partial...");
}

// 2. Add class close and view handlers
const targetClassBtn = `  const foundationClassEditBtn = event.target.closest("[data-foundation-class-edit]");
...
const replaceClassBtn = `  const foundationClassViewBtn = event.target.closest("[data-foundation-class-view]");
...
if (dashCode.includes(targetClassBtn)) {
  dashCode = dashCode.replace(targetClassBtn, replaceClassBtn);
  console.log("Class event handlers added successfully.");
}

fs.writeFileSync("js/dashboard.js", dashCode, "utf8");
console.log("js/dashboard.js saved.");
