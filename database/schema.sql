...
-- ---------------------------------------------------------------------------
-- Mirrored from supabase/migrations/0010_counseling_sacraments_pilot.sql
...
-- Mirrored from supabase/migrations/0011_fevo_prison_materials_pilot.sql
-- ---------------------------------------------------------------------------
-- Backend Phase 11 - F.E.V.O + Prison Ministry + Ministry Materials pilot
...
-- ===========================================================================
-- PHASE 12: Reports + Notifications + Audit Hardening (canonical mirror: 0012)
-- The deployable additive migration contains all compatibility ALTERs/indexes.
-- ===========================================================================
CREATE TABLE IF NOT EXISTS public.report_definitions (id uuid PRIMARY KEY DEFAULT gen_random_uuid(), report_key text UNIQUE NOT NULL, report_name text NOT NULL, description text, module_key text NOT NULL, category text, default_filters jsonb DEFAULT '{}'::jsonb, available_filters jsonb DEFAULT '[]'::jsonb, available_columns jsonb DEFAULT '[]'::jsonb, requires_permission text, sensitivity_level text DEFAULT 'Normal', supports_export boolean DEFAULT true, supports_snapshot boolean DEFAULT true, supports_scheduling boolean DEFAULT false, status text DEFAULT 'Active', metadata jsonb DEFAULT '{}'::jsonb, created_by uuid, updated_by uuid, created_at timestamptz DEFAULT now(), updated_at timestamptz DEFAULT now());
CREATE TABLE IF NOT EXISTS public.saved_report_views (id uuid PRIMARY KEY DEFAULT gen_random_uuid(), report_definition_id uuid REFERENCES public.report_definitions(id), report_key text NOT NULL, view_name text NOT NULL, description text, owner_user_id uuid, owner_name text, church_id uuid REFERENCES public.churches(id), church_name text, scope text DEFAULT 'Private', filters jsonb DEFAULT '{}'::jsonb, columns jsonb DEFAULT '[]'::jsonb, sort_config jsonb DEFAULT '{}'::jsonb, is_default boolean DEFAULT false, is_favorite boolean DEFAULT false, status text DEFAULT 'Active', metadata jsonb DEFAULT '{}'::jsonb, created_by uuid, updated_by uuid, created_at timestamptz DEFAULT now(), updated_at timestamptz DEFAULT now());
CREATE TABLE IF NOT EXISTS public.report_snapshots (id uuid PRIMARY KEY DEFAULT gen_random_uuid(), report_definition_id uuid REFERENCES public.report_definitions(id), report_key text NOT NULL, snapshot_title text, snapshot_date timestamptz DEFAULT now(), module_key text, church_id uuid REFERENCES public.churches(id), church_name text, filters jsonb DEFAULT '{}'::jsonb, summary_metrics jsonb DEFAULT '{}'::jsonb, chart_data jsonb DEFAULT '{}'::jsonb, table_preview jsonb DEFAULT '[]'::jsonb, row_count integer DEFAULT 0, sensitivity_level text DEFAULT 'Normal', created_by uuid, created_by_name text, status text DEFAULT 'Active', metadata jsonb DEFAULT '{}'::jsonb, created_at timestamptz DEFAULT now(), updated_at timestamptz DEFAULT now());
CREATE TABLE IF NOT EXISTS public.report_export_jobs (id uuid PRIMARY KEY DEFAULT gen_random_uuid(), report_key text NOT NULL, report_name text, requested_by uuid, requested_by_name text, church_id uuid REFERENCES public.churches(id), church_name text, export_format text DEFAULT 'CSV', filters jsonb DEFAULT '{}'::jsonb, columns jsonb DEFAULT '[]'::jsonb, status text DEFAULT 'Queued', file_document_id uuid, file_name text, file_url text, storage_path text, row_count integer DEFAULT 0, started_at timestamptz, completed_at timestamptz, failed_at timestamptz, error_message text, sensitivity_level text DEFAULT 'Normal', metadata jsonb DEFAULT '{}'::jsonb, created_at timestamptz DEFAULT now(), updated_at timestamptz DEFAULT now());
CREATE TABLE IF NOT EXISTS public.notification_templates (id uuid PRIMARY KEY DEFAULT gen_random_uuid(), template_key text UNIQUE NOT NULL, template_name text NOT NULL, module_key text, category text, title_template text NOT NULL, message_template text, default_priority text DEFAULT 'Normal', default_type text DEFAULT 'Info', default_action_label text, is_active boolean DEFAULT true, metadata jsonb DEFAULT '{}'::jsonb, created_by uuid, updated_by uuid, created_at timestamptz DEFAULT now(), updated_at timestamptz DEFAULT now());
CREATE TABLE IF NOT EXISTS public.notification_preferences (id uuid PRIMARY KEY DEFAULT gen_random_uuid(), user_id uuid NOT NULL, module_key text, category text, in_app_enabled boolean DEFAULT true, email_enabled boolean DEFAULT false, sms_enabled boolean DEFAULT false, whatsapp_enabled boolean DEFAULT false, push_enabled boolean DEFAULT false, quiet_hours_start text, quiet_hours_end text, frequency text DEFAULT 'Immediate', metadata jsonb DEFAULT '{}'::jsonb, created_at timestamptz DEFAULT now(), updated_at timestamptz DEFAULT now());
CREATE TABLE IF NOT EXISTS public.sensitive_access_events (id uuid PRIMARY KEY DEFAULT gen_random_uuid(), access_type text NOT NULL, module_key text NOT NULL, entity_type text, entity_id uuid, actor_user_id uuid, actor_name text, actor_role text, church_id uuid REFERENCES public.churches(id), church_name text, sensitivity_level text, reason text, allowed boolean DEFAULT false, denied_reason text, field_names jsonb DEFAULT '[]'::jsonb, metadata jsonb DEFAULT '{}'::jsonb, created_at timestamptz DEFAULT now());
CREATE TABLE IF NOT EXISTS public.system_events (id uuid PRIMARY KEY DEFAULT gen_random_uuid(), event_key text, event_type text, module_key text, title text, message text, severity text DEFAULT 'Info', status text DEFAULT 'Open', source text, source_id uuid, metadata jsonb DEFAULT '{}'::jsonb, created_at timestamptz DEFAULT now(), resolved_at timestamptz, resolved_by uuid, resolved_by_name text);
CREATE TABLE IF NOT EXISTS public.data_source_health_checks (id uuid PRIMARY KEY DEFAULT gen_random_uuid(), data_source text NOT NULL, module_key text, status text DEFAULT 'Unknown', checked_at timestamptz DEFAULT now(), latency_ms integer, message text, details jsonb DEFAULT '{}'::jsonb, metadata jsonb DEFAULT '{}'::jsonb);
...
COMMENT ON TABLE public.report_snapshots IS 'Sanitized aggregates only; excludes full confidential notes, salary details, and proof files.';
...
-- ============================================================================
-- D.O.P.: complete Prison Ministry operational collections (migration 0039)
-- ============================================================================
